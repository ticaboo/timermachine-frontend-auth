...
class YTPlayer extends Component {
  ...
  videoPlayingChange = (msg, data) => {
    //console.log('change', this.state.playing, data);
    if (this.state.playing && data === false) this.handlePlayPause();
    if (this.state.playing === false && data === true)
      this.setState({ playing: true });
  };

  videoUrlChange = (msg, data) => {
    // ok, this is currently wonk. i am setting the url when playing over eagerly.
    // console.log('videoUrlChange url: ', this.state.url, ' data: ', data);
  ...
  //todo: mount/constructor:
  videoPlayChangeToken = PubSub.subscribe(VIDEO_PLAY, this.videoPlayingChange);
  videoIdChangeToken = PubSub.subscribe(VIDEO_ID, this.videoUrlChange);
